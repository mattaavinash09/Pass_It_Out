...
namespace Pass_It_Out.Services.PostServices
{
    public class PostService : IPost
    {
        ...
        {
            var friends = ctx.Friends.Where(f => f.UserId == UserId 
            || (f.FriendId == UserId && f.Status == Common.Constants.FriendStatus.Active)).Select(f=>f.FriendId).ToList();

            var listOfAll = ctx.Posts.Where(p => p.PostTo == "2" && p.UserId != UserId && p.Status).ToList();
            
            var listOfOnlyFriends=ctx.Posts.Where(p=>p.PostTo=="1" && friends.Contains(p.UserId) && p.Status).ToList();
